...
import { getUsageTotals } from "@/lib/cost";

export const dynamic = "force-dynamic";

// GET /api/usage?session_id=123 — per-session token/cost totals for the meter.
export async function GET(req: NextRequest) {
  ...
  const totals = await getUsageTotals(sessionId);
